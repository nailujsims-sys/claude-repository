...
| `0015_finance_data_control.sql` | Datenkontrolle: `finance_relation_reactivatable(uuid, uuid)` (`security invoker`, `stable` — darf eine Buchung, die eine Relation aus der Auswertung genommen hat, wieder zählen? Nur wenn sie gerade wirklich nicht zählt, `finance_transaction_protected` falsch ist, keine andere nicht-abgelehnte Relation sie in ihrem `evidence.analytics_deactivated` führt, **und** sie nicht Vorgänger einer anderen **bestätigten** Ablösung ist — der Kettenfall, den die Buchführung allein nicht sieht, weil eine Bestätigung eine bereits stillgelegte Zeile nicht in ihre eigene Liste schreibt); `finance_delete_transaction(uuid)` (`security invoker` — die DELETE-Policy aus 0008 und die `on delete cascade`-Fremdschlüssel erledigen die Arbeit; zusätzlich räumt sie die drei Dinge auf, die kein Fremdschlüssel aufräumen kann: eine Relation, die durch das Löschen unter zwei Mitglieder fällt, einen OFFENEN Prüfposten, der seine letzte Buchung verliert, und — vor dem Löschen der Relation gelesen — die von ihr stillgelegten **überlebenden** Buchungen, die über `finance_relation_reactivatable` wieder eingeschaltet werden. Import, Händler, Muster, Kategorieregeln und das KI-Gedächtnis bleiben — `finance_ai_learning_memories.source_transaction_id` steht seit 0012 auf `set null`, und diese Semantik wird ausdrücklich nicht verschärft) und `finance_reset_user_data()` (`security definer` mit `set search_path = ''`, **ohne Parameter** — die Benutzer-ID kommt ausschließlich aus `auth.uid()`, jede Anweisung trägt `user_id = v_user`, `anon` hat kein `execute`. Sie löscht alle 15 persönlichen `finance_*`-Tabellen in Fremdschlüssel-Reihenfolge und stellt danach über `finance_apply_category_taxonomy` die vollständige Standardtaxonomie wieder her — 9 + 26 = 35 Kategorien; das ist auch der Grund für `definer`: die Taxonomie-Prozedur nimmt eine Benutzer-ID und ist deshalb für `authenticated` gesperrt). Keine Tabelle, keine Spalte, zweimal einspielbar |
