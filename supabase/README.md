...
- **`on delete restrict`** — eine Oberkategorie, an der Unterkategorien hängen,
  verschwindet nicht; sonst blieben Blätter ohne Überschrift zurück und jede
  Aggregation über `parent_id` hätte eine Lücke. Für das Löschen eines
  `auth.users`-Datensatzes ist das unkritisch: `user_id` kaskadiert, Eltern und
  Kinder verschwinden in derselben Anweisung, und die referentielle Prüfung
  läuft an deren Ende.
