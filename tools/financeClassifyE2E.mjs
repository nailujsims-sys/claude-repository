...
if (!bin) {
  console.error('finance classify e2e: RLS_TEST_REQUIRED ist gesetzt, aber es wurde kein unterstütztes PostgreSQL (16, 15 oder 14) gefunden.')
  process.exit(1)
}
...
if (asRoot && !sudoUser) {
  ...
  if (process.env.RLS_TEST_REQUIRED) {
    console.error('finance classify e2e: läuft als root und findet kein unprivilegiertes Konto — im Deployment ist das ein Fehler.')
    process.exit(1)
  }
